...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.keras_input_tensor=Input(shape=(self.exp.input_size[1],self.exp.input_size[0],3))
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False):
        if self.rank == 0:
            ...
            if self.keras_model==None:
                y=self.model.keras_tensor(self.keras_input_tensor)
                self.keras_model=Model(inputs=[self.keras_input_tensor],outputs=y)
            else:
                self.model.keras_update()
                self.keras_model.save(os.path.join(self.file_name,"yolox.h5"))
            if self.args.logger == "wandb":
